...
#Ben added to print shapes
  print(
    f"[HB INIT] CID={self.cid}",
    f"FILE={info['input_file']}",
    f"NHRU={len(self.input_fp.dimensions['hru'])}",
    flush=True
)
 ...
 def initialize_richards(self,vsp_flag,ncsbasins): #laura, svp
  ...
  if self.flagcmatrix==False:  
   ...
   for i in range(1,(int(ncsbasins)+1)):
    ...
    nrow = len(self.input_fp.groups[text].variables['indptr'][:]) - 1     #Ben added
    ...
    a=sparse.csr_matrix((self.input_fp.groups[text].variables['data'][:],
                                   ...
                                   shape=(nrow, nrow),   #Ben added due to shape diff
                                   dtype=np.float64)

                                   #shape=(int((np.max(self.input_fp.groups[text].variables['indices']))+1),int((np.max(self.input_fp.groups[text].variables['indices']))+1)),dtype=np.float64) #Ben comment
    w_dict[text2]=np.array(a.todense())

    with np.errstate(invalid='ignore', divide='ignore'):
     tmp = self.richards.area[basin_aux:basin_aux+nrow] / w_dict[text2]     #Ben added to solve shp error

     #tmp = (self.richards.area[basin_aux:int((np.max(self.input_fp.groups[text].variables['indices']))+1)+basin_aux])/w_dict['Basin%s' %int(i)]  #Ben commented due to shp diff
    dx = (tmp + tmp.T)/2
    self.richards.dx[text2] = dx
    #basin_aux=basin_aux+w_dict[text2].shape[0]   #Ben commented
    basin_aux = basin_aux + nrow    #Ben added
 ...
 def update_input(self,date):
  ...
  print(
      f"[HB INPUT] CID={self.cid}",
      f"itime={self.itime}",
      f"NOAHMP={len(self.noahmp.lwdn)}",
      f"METEO={self.meteodata['lwdown'].shape[1]}",
  flush=True
  )
