...
class HydroBlocks:
 ...
 def initialize_richards(self,vsp_flag,ncsbasins): #laura, svp
  ...
  if self.flagcmatrix==False:  
   ...
   for i in range(1,(int(ncsbasins)+1)):
    ...
    text2='Basin%s' %int(i)
    # Fix rare CSR matrix bug:    Ben comments
    # some basins (e.g., CID 1127 Basin2) have max(indices)+1 < len(indptr)-1.
    # Use indptr to determine matrix size because CSR row count = len(indptr)-1.
    #-------------------------------------------- Ben added
    if int(np.max(self.input_fp.groups[text].variables['indices'][:]))+1 != \
        len(self.input_fp.groups[text].variables['indptr'][:])-1:
        print("FIXING:", self.cid, text, flush=True)
    #--------------------------------------------
    a=sparse.csr_matrix((self.input_fp.groups[text].variables['data'][:],
                                   ...
                                   shape=(max(int(np.max(self.input_fp.groups[text].variables['indices'][:]))+1,len(self.input_fp.groups[text].variables['indptr'][:])-1),    
                                          max(int(np.max(self.input_fp.groups[text].variables['indices'][:]))+1,len(self.input_fp.groups[text].variables['indptr'][:])-1)),dtype=np.float64) # Ben added this line and the line above it and commented the original below (CSR shape fix. CID 1127 Basin2 had max(indices)+1=1 but len(indptr)-1=2, causing "index pointer size" crash, use CSR row count (len(indptr)-1) instead of max(indices)+1 to avoid sparse matrix shape mismatch.)
 ...
 def update_noahmp(self,date,vsp_flag=True): #laura, svp
  ...
  n.update(n.z_ml,n.dt,n.lwdn,n.swdn,n.u_ml,n.v_ml,n.q_ml,n.t_ml,n.prcp,n.psfc,\
           ...
           n.apar,n.psn,n.sav,n.sag,n.rssun,n.rssha,n.swdn_3d,n.lwdn_3d,\
 ...
 def update_output(self,date):
  ...
  tmp['lwdn_3d'] = np.copy(NOAH.lwdn_3d) #W/m2
 ...
 def create_netcdf_file(self,):
  ...
  metadata = {
	     ...
	     "fvv":{'description':'Vegetated friction velocity','units':'m/s','dims':('time','hru',),'precision':4},
             ...
             "swdn_3d":{'description':'Shortwave radiation down with 3D topography correction', 'units':'W/m2', 'dims':('time','hru'), 'precision':4},
             "lwdn_3d":{'description':'Longwave radiation down with 3D topography correction', 'units':'W/m2', 'dims':('time','hru'), 'precision':4},
