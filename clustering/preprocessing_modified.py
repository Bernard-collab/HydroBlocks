...
def Postprocess_Input(rdir,edir,cids,rank,size,comm):
 ...
 if rank == 0:
  ...
  os.makedirs(sdir, exist_ok=False)  #  Create parent explicitly
  
  for var in vars:
   os.makedirs(os.path.join(sdir, var), exist_ok=False)  # Fail if exists
 comm.Barrier()
 #Ben edits ended here

 #Create cid, hru, and channel maps
 #vars = ['cids','cids_org','dem','hrus','channels','hand','basins','basin_clusters'] #Ben commented, replaced with the one directly under where edit start
 ...
 if rank == 0:
  for var in vars:
   ...
   os.system('/projects/battobrah@xsede.org/software/anaconda/envs/HB3/bin/gdalbuildvrt %s/%s.vrt %s/%s/*.tif' % (sdir,var,sdir,var))
